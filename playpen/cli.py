...
def _with_model_registry_file(model_registry_file: Path) -> Optional[callable]:
    ...
    target = Path(__file__).resolve().parent.parent / "model_registry.json"
    ...
    lock_path = target.parent / ".playpen_model_registry.lock"
...
def evaluate_suite_moe(
    ...
    if dataset_name is not None:
        ...
        for game_name in sorted(rows_by_game.keys()):
            game_rows = rows_by_game[game_name]
            if not game_rows:
                continue
            ...
            for row in game_rows:
                exp = _row_get(row, "experiment", "experiment_name", default=None)
                tid = _row_get(row, "task_id", "game_id", "instance_id", default=None)
                if tid is None:
                    continue
                routing_manifest["assignments"].append(
                    {
                        "game": game_name,
                        "experiment": str(exp) if exp is not None else None,
                        "task_id": int(tid),
                        "expert_model": expert,
                        "router": "loss_router",
                        "routing_scope": "game",
